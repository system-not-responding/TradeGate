...
# below is chopping if elses
        ...
        print(f"Your risk-to-reward does not meet the minimum requirement: {round(risk_ratio, 2)} / 2. You cannot place a trade\n")    
else:
...
# -------------------------------------------------------
# here im trying to get the program to exit/restart based on user input
exit_program = "'q'".lower()
restart_program = "'e'"
exit_or_restart_program = input(f"Would you like to restart?\n>>> ")
...
def main():
    ...
    reward_one_more_contract = float(
        "{:.2}".format((contract_number + 1) * tick_value * tick_reward))  # calculates reward with one more contract
    reward_one_less_contract = float("{:.2}".format((contract_number - 1) * tick_value * tick_reward))
    ...
    while True:
        if exit_or_restart_program == "yes" or exit_or_restart_program == "y":
            print("\n")
            main()
        else:
            break
